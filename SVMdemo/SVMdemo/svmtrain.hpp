...
void parse_command_line(int elem);
...
svm_model* SVMTRAIN(int SIZE, int ELEM, const char* TRAINPATH, const char* MODELPATH)
{
	...
	parse_command_line(ELEM);
	read_problem(TRAINPATH);
	...
	else
	{
		...
		if (svm_save_model(MODELPATH, model))
		{
...
void parse_command_line(int elem)
{
	...
	param.degree = 3;
	param.gamma = 1.0 / (elem - 1);	// 1/num_features
	param.coef0 = 0;
	param.nu = 0.5;
...
/*	for(i=1;i<argc;i++)
	...
	//strcpy(model_file_name,MODELPATH);
}
